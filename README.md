# Проектная работа "Веб-ларек"

Стек: HTML, SCSS, TS, Webpack

Структура проекта:

- src/ — исходные файлы проекта
- src/components/ — папка с JS компонентами
- src/components/events — папка с событиями приложения
- src/components/model — папка с моделями
- src/components/view — папка с представлениями
- src/components/base/ — папка с базовым кодом

Важные файлы:

- src/pages/index.html — HTML-файл главной страницы
- src/types/index.ts — файл с общими типами
- src/index.ts — точка входа приложения
- src/scss/styles.scss — корневой файл стилей
- src/utils/constants.ts — файл с константами
- src/utils/utils.ts — файл с утилитами

## Установка и запуск

Для установки и запуска проекта необходимо выполнить команды

```
npm install
npm run start
```

или

```
yarn
yarn start
```

## Сборка

```
npm run build
```

или

```
yarn build
```

## Описание проекта:

Проект представляет собой интернет-магазин для веб-разработчиков под названием "Веб-ларек". Это место, где пользователи
могут:

Просматривать широкий ассортимент товаров, предназначенных для веб-разработки.

Добавлять товары в корзину для удобства дальнейшей покупки.

Оформлять заказы и совершать покупки через удобный интерфейс.

## Архитектура проекта (MVC):

Архитектура MVC помогает разделить ответственность между слоями и делает код более поддерживаемым и тестируемым.

## Описание базовых классов:

---
### [AModel](src/components/base/AModel.ts)

Родительский класс моделей.

***Поля***:
Не имеет.

***Конструктор***:
Принимает параметр типа IEvents.

***Методы***:
- `emitChanges`- Принимает параметром событие event типа string и payload типа object(необязательный. Если не передам будет использоваться пустой объект). Генерирует событие.

---
### [Api](src/components/base/Api.ts)

Базовый класс для работы с REST API.
Позволяет совершать GET, POST, PUT, DELETE-запросы.

***Поля***:

- `baseUrl` - Базовый URL, к которому будут добавляться относительные URI, типа string.
- `options` - Типа RequestInit. Объект с параметрами по умолчанию для всех запросов.

***Конструктор***:
Принимает в параметры базовый URL для всех запросов: baseUrl типа string.
И options: необязательный объект конфигурации типа RequestInit используется для установки заголовков запроса.
Автоматически устанавливает заголовок 'Content-Type': 'application/json'.

***Методы***:
- `handleResponse(response: Response): Promise<T>`- Внутренний метод: обработчик ответа сервера. Принимает параметром response типа Response — объект ответа, полученный от fetch. Возвращает Promise<T> — успешный результат или ошибка
- `get(uri: string)`- принимает изменяющеюся часть url-адреса uri типа string, возвращает ответ от сервера.
- `post(uri: string, data: object, method: ApiPostMethods = 'POST')`- принимает изменяющеюся часть url-адреса,
  принимает данные в виде объекта для отправки на сервер, type ApiPostMethods = 'POST' | 'PUT' | 'DELETE'..

***Типы***:
- `ApiListResponse<Type>`
  Универсальный тип ответа для списков:
```
{
  total: number;
  items: Type[];
}
```

- `ApiPostMethods` - Допустимые HTTP-методы: `'POST' | 'PUT' | 'DELETE'`

---

### [AView](src/components/base/AView.ts)

Обеспечивает базовые методы для работы с DOM и логику обновления UI на основе данных.
Наследуется другими представлениями, которые реализуют собственную отрисовку через метод doRender. Имеет обобщенный тип `RenderData`.

***Поля***:

- `data: Partial<RenderData>` - Внутреннее хранилище данных, используемых для рендеринга.
- `container: HTMLElement` - Контейнер DOM-элемента, к которому привязано представление.

***Конструктор***:

Принимает параметром container: HTMLElement — DOM-элемент, в котором происходит отрисовка.

***Методы***:
- `render(data: Partial<RenderData>): HTMLElement` - Объединяет переданные данные с текущими, вызывает метод doRender и возвращает DOM-элемент.
  Используется для обновления UI.

- `toggleClass(element: HTMLElement, className: string, force?: boolean): void` - Добавляет или убирает CSS-класс на DOM-элементе.
  Принимает параметрами: element — DOM-элемент, className — название класса, force — явное указание: добавить (true) или убрать (false) класс.

- `setText(element: HTMLElement, value: unknown): void` - Устанавливает текстовое содержимое элемента.
  Принимает параметрами: element — DOM-элемент, value — текст (любого типа, преобразуется в строку).

- `setDisabled(element: HTMLElement, state: boolean): void` - Блокирует или разблокирует элемент формы.
  Параметры: element — DOM-элемент, state — true, если нужно заблокировать.

- `setImage(element: HTMLImageElement, src: string, alt?: string): void` - Устанавливает src и alt для изображения.
  Параметры: element — DOM-изображение, src — ссылка на изображение, alt — альтернативный текст (необязательно).

- `doRender(data: Partial<RenderData>): void` - Абстрактный метод. Выполняет отрисовку представления по данным.

---

### [EventEmitter](src/components/base/Events.ts)

Реализует паттерн «Observer/Наблюдатель» и обеспечивает работу событий, его методы позволяют устанавливать и снимать слушатели событий, вызвать слушатели при возникновении события.

***Поля***:
- `_events: Map<EventName, Set<Subscriber>>` - Хранилище всех подписчиков. Каждому имени события соответствует набор его обработчиков (функций).

***Конструктор***:
Создаёт пустой список событий. Инициализирует _events.

***Методы***:
- `on<T>(event: EventName, callback: (data: T) => void): void` - Подписывает обработчик на конкретное событие.
  Параметрами принимает event: string | RegExp — имя события или регулярное выражение и callback: (data: T) => void — функция, вызываемая при срабатывании события.

- `off(event: EventName, callback: Subscriber): void` - для отписки от события. Параметрами принимает event: string | RegExp — имя события или регулярное выражение
  и callback: Function — ранее добавленный обработчик.

- `emit<T>(event: string, data?: T): void` - уведомления подписчиков о наступлении события. Параметрами принимает event: string — имя события
  и data: T (необязательное) — данные, передаваемые обработчикам.

- `onAll(callback: (event: EmitterEvent) => void): void` - Для подписки на все события. Парметры: callback: (event: { eventName: string, data: unknown }) => void.
- `offAll(): void` - Для сброса всех подписчиков.
- `trigger<T>(event: string, context?: Partial<T>): (data: T) => void` - генерирует заданное событие с заданными аргументами. Это позволяет передавать его в качестве обработчика
  события в другие классы. Эти классы будут генерировать события, не будучи при этом напрямую зависимыми от класса
  EventEmitter. Параметрами принимает event: string — имя события и context: Partial<T> (необязательно) — дополнительные данные, объединяемые с data. Возвращает (data: T) => void — колбэк, который можно передать как обработчик в UI-компоненты

---

## Описание данных (Интерфейсы):

---
### [Product](src/types/index.ts)
Представляет данные одного товара в каталоге
- `id: string` - уникальный идентификатор товара. Используется для поиска, удаления, добавления в корзину и связи с заказами
- `description: string` - текстовое описание товара
- `image: string` - ссылка на изображение товара
- `title: string` - название товара
- `category: ProductCategory` - категория товара
- `price: number` - стоимость товара
---
### [OrderResult](src/types/index.ts)
Хранит данные о заказе после его успешного оформления.
- `id: string` - id заказа
- `total: number` - общая сумма заказа
---
### [IOrder](src/types/index.ts)
Представляет полные данные для оформления заказ.
- `email: string` — электронная почта покупателя
- `phone: string` — номер телефона покупателя
- `address: string` — адрес доставки или самовывоза
- `payment: Payment | null` — выбранный способ оплаты
- `total: number` — итоговая сумма всех выбранных товаров
- `items: string[]` — список ID товаров
---
### Тип данных [Payment](src/types/index.ts)
Определяет способ оплаты заказа.
- `card` - онлайн
- `cash` - при получении
---
### Тип данных [ProductCategory](src/types/index.ts)
Определяет категорию товара для его классификации.
- `софт-скил` - товары, связанные с развитием "мягких навыков" (soft skills)
- `хард-скил` - товары, относящиеся к "жёстким навыкам" (hard skills)
- `дополнительное` - товары, которые не входят в основные категории
- `кнопка` - возможно, специфическая категория товаров в проекте
- `string` - допускается любое строковое значение для расширяемости
- 
---

### [ILarekApi](src/components/base/Api.ts)

Интерфейс Контракта API-клиента для взаимодействия с сервером магазина.

***Методы***:
- `getProductItem`- загружает товара с сервера по id. Возвращает объект Product.
- `getProductList`- загружает список всех товаров с сервера. Возвращает массив объектов Product[].
- `orderProducts`- Отправляет успешный заказ на сервер. Принимает объект order типа IOrder. Возвращает результат заказа типа OrderResult.

### [IBasketData](src/components/model/BasketModel.ts)

Интерфейс данных корзины.
Данные, которые посылаются вместе с событием basketEvents.CHANGED.

***Поля***:
- `products`: Product[]; - список товаров в корзине
- `total`: number; - общая сумма товаров

---

### [IOrderChange](src/components/model/OrderModel.ts)

Интерфейс данных заказа. 1 шаг.
Данные, которые посылаются вместе с событием CheckoutEvent.ORDER_CHANGED.

***Поля***:
- `payment?`: Payment; - Выбранный способ оплаты. Может отсутствовать, если пользователь его ещё не указал.
- `address`: string; - адрес доставки
- `valid`: boolean; - Флаг, указывающий валидны ли оба поля..
- `error`: string; - Сообщение об ошибке.

---

### [IContactsChange](src/components/model/ContactsModel.ts)

Интерфейс данных заказа. 2 шаг.
Данные, которые посылаются вместе с событием CheckoutEvent.CONTACTS_CHANGED.

***Поля***:
- `email`: string; - Email пользователя.
- `phone`: string; - телефон пользователя.
- `valid`: boolean; - Флаг, указывающий валидны ли оба поля.
- `error`: string; - Сообщение об ошибке.

---

### [ISuccess](src/components/model/SuccessModel.ts)

**Интерфейс успешного заказа. ISuccess**.
Данные, которые посылаются вместе с событием SuccessEvent.CHANGED.

***Поля***:
- `total` - Сумма заказа.

---

### [IEvents](src/components/base/Events.ts)

Типизированный контракт для брокера событий (EventEmitter), включая методы подписки, отписки, вызова и генерации событий.

***Методы***:
- `on<T extends object>(event: EventName, callback: (data: T) => void): void;` - Подписка на событие по имени или шаблону
- `emit<T extends object>(event: string, data?: T): void;` - Вызывает всех соответствующих подписчиков.
- `trigger<T extends object>(event: string, context?: Partial<T>): (data: T) => void;` - Создаёт функцию-триггер, которая генерирует событие при вызове

***Алиасы типов***
- `type EventName = string | RegExp` - Имя события. Может быть строкой или регулярным выражением для шаблонной подписки.
- `type Subscriber = Function` - Тип любого обработчика события (подписчика).
- `type EmitterEvent = { eventName: string; data: unknown }` - Тип данных, передаваемых в обработчик при глобальной подписке через onAll.
  Содержит имя события и переданные данные.

---
### [IPage](src/components/view/PageView.ts)

Определяет структуру данных, которые принимает метод render:

- `counter: number` — количество товаров в корзине;

- `catalog: HTMLElement[]` — список DOM-элементов каталога;

- `locked: boolean` — флаг блокировки страницы (например, при открытии модального окна).
---
### [IPageActions](src/components/view/PageView.ts)

Определяет список доступных действий, передаваемых в конструктор:

- `onBasketClick(event: MouseEvent): void` — обработчик клика по иконке корзины.

---
### [ICard](src/components/view/CardView.ts)

Определяет структуру данных, которые принимает метод render:

- `id: string` — уникальный идентификатор товара;

- `category: ProductCategory` — категория товара ('софт-скил', 'хард-скил', 'дополнительное', 'кнопка', и др.);

- `title: string` — заголовок карточки;

- `image: string` — путь к изображению;

- `price: number | null` — цена в синапсах или null, если "бесценно";

- `description: string` — описание товара.

---
### [ICardActions](src/components/view/CardView.ts)

Объект с действиями, передаваемый в конструктор:

- `onClick(event: MouseEvent): void` — обработчик клика по карточке или кнопке.

---
### [ICardPreview](src/components/view/CardPreviewView.ts)

расширяет интерфейс ICard и добавляет информацию о товарах, находящихся в корзине:
- `basketProducts: Product[]` — Список товаров в корзине.

---
### [IModalData](src/components/view/ModalView.ts)

Определяет структуру данных, которые передаются в метод render
- `content: HTMLElement` — DOM-элемент, отображаемый внутри модального окна
---
### [IModalActions](src/components/view/ModalView.ts)

Описывает доступные действия, передаваемые в конструктор
- `onClose: () => void` — Колбэк, вызываемый при закрытии модального окна
---
### [ISuccess](src/components/model/SuccessModel.ts)

Определяет структуру данных, которые принимает метод render
- `total: number` — итоговая сумма списанных синапсов
---
### [ISuccessAction](src/components/view/SuccessView.ts)

Описывает список действий, передаваемых в конструктор:
- `onSuccessButtonClick: (event: MouseEvent) => void` — обработчик клика по кнопке "Закрыть"
---
### [IOrder](src/components/view/OrderView.ts)

Описывает структуру данных, которые принимает метод render.
- `valid: boolean` — валидна ли форма
- `payment: Payment` — способ оплаты ('card' или 'cash')
- `address: string` — введённый адрес
- `error: string` — текст ошибки
---
### [IOrderActions](src/components/view/OrderView.ts)

Определяет список действий, передаваемых в конструктор:

- `onOnlineClick: (event: MouseEvent) => void` — обработчик клика по кнопке "Картой"
- `onOfflineClick: (event: MouseEvent) => void` — обработчик клика по кнопке "Наличными"
- `onProceedButtonClick: (event: MouseEvent) => void` — обработчик клика по кнопке "Далее"
- `onAddressInput: (event: Event) => void` — обработчик ввода в поле адреса
---
### [IContact](src/components/view/ContactsView.ts)

Описывает структуру данных, которые принимает метод render

- `valid: boolean` — валидна ли форма (включать/отключать кнопку)
- `email: string` — значение поля email
- `phone: string` — значение поля телефона
- `error: string` — текст ошибки
---
### [IContactActions](src/components/view/ContactsView.ts)

- `onEmailInput: (event: Event) => void` — при вводе email
- `onPhoneInput: (event: Event) => void` — при вводе телефона
- `onToPayButtonClick: (event: MouseEvent) => void` — при клике на кнопку "К оплате"
---
### [IBasketView](src/components/view/BasketView.ts)

Описывает структуру данных, которые принимает метод render:

- `items: HTMLElement[]` — список DOM-элементов товаров в корзине
- `total: number` — общая сумма заказа
---
### [IBasketAction](src/components/view/BasketView.ts)

Содержит обработчик действия пользователя:
- `onClick: (event: MouseEvent) => void` — при клике на кнопку "Оформить заказ"
---
### [ICardBasket](src/components/view/CardBasketView.ts)

Описывает модель данных одной карточки в корзине
- `index: number` — Порядковый номер товара в корзине
- `title: string` — Название товара
- `price: number` — Цена товара
---
### [ICardBasketActions](src/components/view/CardBasketView.ts)

Определяет обработчики событий, которые может вызвать представление:
- `onCardButtonDeleteClick: (event: MouseEvent) => void` — Обработчик удаления товара
---

## Модели данных:

### [CatalogModel](src/components/model/CatalogModel.ts)

Наследуется от абстрактной модели данных AModel. Сообщает всем подписчикам, что товары пришли с сервера и наш каталог товаров изменился.

***Поля***:
Не имеет.

***Конструктор***:
Наследует от абстрактной модели данных AModel.

***Методы***:

- `setProducts` - оповещает всех подписчиков об изменении каталога товаров;
  Принимает параметром массив товаров, которые пришли с сервера.

---
### [BasketModel](src/components/model/BasketModel.ts)

Модель данных корзины интернет-магазина.
Реализует хранение товаров и управление товарами в корзине.
Наследуется от абстрактной модели данных AModel.
Имеет событийную модель для уведомления подписчиков об изменениях.

***Поля***:
- `products` хранит массив товаров типа Product[].
-
***Конструктор***:
Наследует конструктор от абстрактной модели данных AModel.

***Методы***:
- `getProducts` - Возвращает массив объектов товаров типа Product[].
- `getTotal` - Возвращает сумму цен товаров в корзине типа number.
- `add` - Метод принимает параметр product типа Product. Добавляет товар в корзину, конкретно в наш массив, в поле products
  и сообщает об изменениях подписчикам.
- `remove` - Метод принимает параметр product типа Product. Удаляет товар из корзины, конкретно из нашего массива, в поле products
  и сообщает об изменениях подписчикам.
- `clear` - Удаляет все товары из корзины, очищает наш массив в поле products.
  и сообщает об изменениях подписчикам.
- `changed` - Внутренний метод, вызываемый после каждого изменения. Генерирует событие CHANGED с актуальными данными корзины.
---
### [OrderModel](src/components/model/OrderModel.ts)

Модель данных заказа товаров. 1 шаг.

Реализует хранение данных о выбранном способе оплаты и адресе.
Наследуется от абстрактной модели данных AModel.

***Поля***:
- `_payment` хранит способ оплаты выбранный пользователем типа Payment = 'card' | 'cash';.
- `_address` хранит адрес пользователя типа string, изначально пустая строка.

***Конструктор***:
Наследует конструктор от абстрактной модели данных AModel.

***Методы***:
- `getPayment` - Возвращает способ оплаты, выбранный пользователем типа Payment. Может не иметь значения.
- `getAddress` - Возвращает адрес, введенный пользователем типа string.
- `setPayment` - Принимаем параметром способ оплаты, типа Payment. Добавляет его в поле _payment. Сообщает подписчикам об изменениях.
- `getValid` - Возвращает true или false типа boolean, если пользователь выбрал способ оплаты и ввел адрес.
- `setAddress` - Принимает адрес типа string, введенный пользователем. Добавляет его в поле _address. Сообщает подписчикам об изменениях.
- `getError` - Возвращает первую подходящую ошибку типа string если не выбран способ оплаты и не заполнено поле с адресом.
- `orderChanged` - Внутренний метод, вызываемый после каждого изменения. Генерирует событие ORDER_CHANGED с актуальными данными заказа.

---
### [ContactsModel](src/components/model/ContactsModel.ts)

Модель данных заказ товаров. 2 шаг.

Реализует хранение данных Email и телефон.
Наследуется от абстрактной модели данных AModel.

***Поля***:
- `_email` - хранит email пользователя типа string, изначально пустая строка.
- `_phone` - хранит телефон пользователя типа string, изначально пустая строка.

***Конструктор***:
Наследует конструктор от абстрактной модели данных AModel.

***Методы***:

- `getEmail` - Возвращает Email, введенный пользователем типа string.
- `getPhone` - Возвращает телефон, введенный пользователем типа string.
- `setEmail` - Принимает Email типа string, введенный пользователем. Добавляет его в поле _email. Сообщает подписчикам об изменениях.
- `setPhone` - Принимает телефон типа string, введенный пользователем. Добавляет его в поле _phone. Сообщает подписчикам об изменениях.
- `contactsChanged` - Внутренний метод, вызываемый после каждого изменения. Генерирует событие CONTACTS_CHANGED с актуальными данными заказа.
- `getValid` - Проверяет, заполнены ли оба поля — email и телефон.
- `getError` - Возвращает первую подходящую ошибку типа string если не заполнено поле c телефоном или с Email.
---
### [SuccessModel](src/components/model/SuccessModel.ts)

Модель данных успешного заказа.
Реализует хранение данных суммы заказа.
Наследуется от абстрактной модели данных AModel.

***Поля***:
- `total` - хранит сумму заказа. Тип number. Изначально равен 0.

***Конструктор***: Не имеет.
***Методы***:
- `setTotal` - Принимает сумму заказа типа number. Добавляет его в поле total. Сообщает подписчикам об изменениях: вызывает событие SuccessEvent.CHANGED

---
### [LarekApiModel](src/components/model/LarekApiModel.ts)

Модель данных, которая передаёт и получает данные от сервера.
Наследуется от абстрактной модели данных Api.

***Поля***:
- `cdn` - Базовый URL CDN, добавляемый к путям изображений товаров типа string.

***Конструктор***:
Создаёт экземпляр API-клиента. Принимает параметры:
- `cdn` - типа string. адрес CDN для подстановки к полям image у товаров.
- `baseUrl` - типа string. Базовый URL API.
- `options` - типа RequestInit. Необязательные параметры запроса.

***Методы***:
- `getProductItem` - Принимает айди товара типа string. Возвращает объект Product с добавленным полным URL изображения.
- `getProductList` - Возвращает массив типа Product[] с URL изображения.
- `orderProducts` - Принимает объект типа IOrder. Возвращает результат оформления заказ OrderResult.

---

## Представления

Позволяют отображать элементы страницы с полученными данными, позволяют взаимодействовать с пользователем.
### [PageView](src/components/view/PageView.ts)

Отвечает за отображение главной страницы приложения.
Наследуется от базового класса AView и реализует отрисовку счетчика корзины, каталога и состояния блокировки страницы.

***Конструктор***:

Создаёт экземпляр страницы.
Принимает параметры:
container: HTMLElement — DOM-элемент, в который будет отрисована страница.
actions: IPageActions — объект с колбэком onBasketClick.

Во время инициализации: выбираются элементы .page__wrapper, .header__basket-counter, .gallery, .header__basket с помощью ensureElement;
назначается обработчик клика на иконку корзины.

***Поля***:
- `wrapper: HTMLElement` —обёртка страницы (.page__wrapper);

- `counter: HTMLElement` — элемент с количеством товаров в корзине (.header__basket-counter);

- `catalog:HTMLElement` — контейнер для карточек каталога (.gallery);

- `basket:HTMLElement` — иконка корзины (.header__basket).

***Методы***:
- `doRender(data: Partial<IPage>): void` — Метод, реализующий отрисовку страницы на основе переданных данных.

Обновляет:
счётчик корзины;
список товаров;
состояние блокировки страницы.

- `setLocked(value: boolean): void` — Добавляет или убирает класс page__wrapper_locked у обёртки страницы.
  value — true блокирует страницу, false — разблокирует.

---
### [CardView](src/components/view/card/CardView.ts)

Отвечает за отображение карточки товара на странице. Наследуется от AView и реализует базовую логику отображения данных о товаре: заголовок, изображение, категорию, цену, описание и кнопку.

***Поля***:
- `category:HTMLElement` — DOM-элемент категории.
- `title: HTMLElement` — заголовок.
- `image: HTMLImageElement` — .
- `price: HTMLElement` — изображение товара.
- `description: HTMLElement | null` — цена.
- `button: HTMLButtonElement | null` — кнопка (если есть).

***Конструктор***:

- `constructor(protected container: HTMLElement, actions?: ICardActions)`
  Инициализирует все элементы карточки: .card__category, .card__title, .card__image, .card__price, .card__text, .card__button.
  Если передан onClick, добавляет обработчик: если есть кнопка — на кнопку; если кнопки нет — на всю карточку.

***Методы***:
- `doRender(data: Partial<RenderData>): void` — Обновляет контент карточки: Заголовок; Картинку и alt-текст; Категорию и соответствующий класс; Цена: "бесценно" или в синапсах; Описание.

- `setProductCategory(value: ProductCategory): void` — Устанавливает текст и класс категории, например:
  'хард-скил' → card__category _hard
  'софт-скил' → card__category _soft

- `getProductCategoryClass(value: string): string` — Возвращает соответствующий CSS-класс в зависимости от значения категории.

- `setProductPrice(price: number | null): void` — Если цена null → выводит "Бесценно",
  иначе — выводит ${price} синапсов.

- `setProductDescription(value: string): void` — Обновляет описание, если элемент существует.

***Используемая утилита***

- `ensureElement<T>(selector: string, parent?: HTMLElement): T` — безопасно получает элемент по селектору или выбрасывает ошибку, если не найден.
---
### [CardCatalogView](src/components/view/card/CardCatalogView.ts)

специфическая реализация абстрактного класса CardView и представляет собой каталог карточек товаров. Этот класс может использоваться для отображения списка карточек товаров на странице.
***Поля***:
Не имеет

***Конструктор***:
Наследует от абстрактного класса CardView

***Методы***:
Не имеет

---
### [CardPreviewView](src/components/view/card/CardPreviewView.ts)

наследуется от класса CardView и используется для отображения карточки товара с возможностью взаимодействия с корзиной. Он управляет состоянием кнопки добавления товара в корзину в зависимости от того, есть ли данный товар в корзине или нет.

***Поля***:
Не имеет полей

***Конструктор***:
Наследует от абстрактного класса CardView

***Методы***:
- `doRender(data: Partial<ICardPreview>): void` - Переопределённый метод рендеринга. В дополнение к отрисовке базовых данных карточки товара (категория, название, цена и т.д.), этот метод управляет состоянием кнопки добавления в корзину.

Если цена товара равна null, кнопка отключена с текстом "Не продаётся".

Если товар уже в корзине, кнопка отключена с текстом "Уже в корзине".

Если товар не в корзине, кнопка активна с текстом "В корзину".

- `isAddedToCart(data: Partial<ICardPreview>): boolean`
  Проверяет, добавлен ли товар в корзину. Если товар из карточки уже присутствует в корзине (по совпадению id), то возвращает true, иначе — false.

- `setButtonState(enabled: boolean, text: string): void`
  Управляет состоянием кнопки. Если кнопка активна (enabled = true), она доступна для клика и имеет текст, переданный в text. Если кнопка не активна (enabled = false), она отключена и текст меняется на переданный.

---
### [ModalView](src/components/view/ModalView.ts)

управляет отображением и поведением модального окна. Он отвечает за рендеринг содержимого модального окна, а также за открытие и закрытие модалки. Наследуется от базового класса AView.

***Поля***:
- `closeButton: HTMLButtonElement` — кнопка закрытия модального окна (.modal__close);
- `content: HTMLElement` — контейнер для отображаемого контента (.modal__content).

***Конструктор***:
Создаёт экземпляр модального окна.

Принимает параметрами:
- `container: HTMLElement` — основной DOM-элемент модального окна.
- `actions: IModalActions` — объект с колбэком onClose (в текущей реализации не применяется).

Ищутся элементы .modal__close и .modal__content внутри контейнера с помощью ensureElement;
Назначаются обработчики: Клик по кнопке закрытия (.modal__close) вызывает метод close; Клик по фону (this.container) тоже вызывает close;
Клик по самому контенту (.modal__content) останавливает всплытие события, чтобы избежать закрытия модалки при клике внутри неё.

***Методы***:
- `doRender(data: Partial<IModalData>): void` — Метод для отображения содержимого модального окна.
  Если передан content, вызывается метод setContent.
- `setContent(value: HTMLElement | null): void` — Устанавливает содержимое модального окна. Если передан null, содержимое очищается.
- `open(): void` — Открывает модальное окно — добавляет класс modal_active к контейнеру.
- `close(): void` — Закрывает модальное окно — удаляет класс modal_active и очищает содержимое.

### [SuccessView](src/components/view/SuccessView.ts)

отвечает за отображение экрана успешного оформления заказа.
Показывает сообщение с итоговой суммой списанных синапсов и кнопку закрытия.
Наследуется от базового класса AView

***Поля***:
- `description: HTMLElement` — Элемент с текстом "Списано X синапсов".
- `button: HTMLButtonElement` — Кнопка закрытия сообщения.

***Конструктор***:
Создаёт экземпляр экрана успеха:
Принимает параметры:
- `container: HTMLElement` — DOM-элемент, в который отрисовывается компонент;
- `actions: ISuccessAction` — объект с обработчиком нажатия кнопки закрытия (onSuccessButtonClick).

При инициализации:
Находит и сохраняет .order-success__description и .order-success__close с помощью ensureElement;
Назначает обработчик клика на кнопку закрытия;
Устанавливает начальное значение суммы 0

***Методы***:
- `doRender(data: Partial<ISuccess>): void` — Обновляет отображение компонента. Если передано total, вызывает setTotal.
- `setTotal(total: number): void` — Устанавливает текст с итоговой суммой списанных синапсов.
  Если total > 0, текст: "Списано 1 000 синапсов" (с форматированием через formatNumber);
  Иначе — очищает текст.

### [OrderView](src/components/view/checkout/OrderView.ts)

отвечает за отображение и управление формой оформления заказа.
Поддерживает выбор способа оплаты, ввод адреса доставки, отображение ошибок и управление кнопкой перехода к следующему шагу.
Наследуется от базового класса AView

***Поля***:
- `buttonOnline: HTMLButtonElement` — Кнопка "Онлайн"
- `buttonOffline: HTMLButtonElement`— Кнопка "При получении"
- `addressInput: HTMLInputElement` — Поле ввода адреса
- `proceedButton: HTMLButtonElement` — кнопка "Далее" (submit)
- `errors: HTMLElement` — Элемент для отображения текста ошибки

***Конструктор***:
Создаёт экземпляр формы заказа.
Параметры:
- `container: HTMLFormElement` — HTML-форма, в которую отрисовывается компонент;

- `actions: IOrderActions` — объект с колбэками, обрабатывающими действия пользователя.

При инициализации:
Получает и сохраняет ссылки на элементы:
кнопки оплаты (card, cash), поле ввода адреса, кнопку "Далее", блок ошибок;
Назначает слушатели на все элементы формы.

***Методы***:

- `doRender(data: Partial<IOrder>): void` —
  Обновляет элементы формы на основе переданных данных:
  Включает/отключает кнопку "Далее", если задано valid;
  Устанавливает выбранный способ оплаты, если задано payment;
  Устанавливает значение поля address;
  Отображает сообщение об ошибке, если передано error

- `setPayment(payment: Payment | undefined): void` — Обновляет визуальное состояние кнопок оплаты, добавляя класс button_alt-active активной кнопке.

### [ContactsView](src/components/view/checkout/ContactsView.ts)

отвечает за отображение и поведение второго шага формы оформления заказа — ввода контактной информации.
Позволяет управлять полями email и телефона, отображает ошибки и контролирует активность кнопки "К оплате".
Наследуется от базового класса AView

***Поля***:
- `emailInput: HTMLInputElement` — Поле ввода email
- `phoneInput: HTMLInputElement` — Поле ввода телефона
- `toPayButton: HTMLButtonElement` — Кнопка перехода к успешному заказу
- `errors: HTMLElement` — Элемент для вывода ошибок

***Конструктор***:
Создаёт экземпляр компонента формы контактов.

- `container: HTMLFormElement` — HTML-форма, в которую отрисовывается компонент;
- `actions: IContactActions` — объект с обработчиками событий.

При инициализации:
Получает и сохраняет ссылки на элементы формы:
поле email (input[name=email]);
поле телефона (input[name=phone]);
кнопку "Оплатить" (button[type=submit]);
блок для ошибок (.form__errors);
Назначает обработчики событий на поля ввода и кнопку.

***Методы***:
- `doRender(data: Partial<IContact>): void`
  Метод обновляет состояние формы в зависимости от переданных данных. Обновляет:
  Состояние кнопки "К оплате" (включена/отключена); Значения полей email и телефона;
  Сообщение об ошибке, если оно задано

---
### [BasketView](src/components/view/basket/BasketView.ts)

отвечает за отображение содержимого корзины пользователя, отображение общей суммы и управление состоянием кнопки оформления заказа.
Наследуется от базового класса AView

***Поля***:

- `basketList: HTMLElement` — Контейнер для списка товаров
- `basketTotal: HTMLElement` — Элемент отображения суммы заказа
- `buttonBasket: HTMLButtonElement` — Кнопка оформления заказа

***Конструктор***:
Создаёт экземпляр компонента корзины.

- `container: HTMLElement` — DOM-элемент, в который отрисовывается корзина;
- `actions: IBasketAction` — объект с обработчиком клика по кнопке оформления.
  При инициализации:
  Получает ссылки на элементы: список товаров (.basket__list); блок с общей суммой (.basket__price); кнопку оформления заказа (.basket__button);
  Назначает обработчик клика по кнопке; Инициализирует корзину пустыми данными (items = [], total = 0).

***Методы***:
- `doRender(data: Partial<IBasketView>): void` — Метод обновляет отображение корзины:
  Обновляет: Список товаров; Общую сумму; Состояние кнопки в зависимости от наличия товаров.

Приватные методы
- `setItems(items: HTMLElement[])` — Заменяет содержимое .basket__list; Если список пуст — отображает надпись «Корзина пуста» и отключает кнопку;
  Иначе — выводит список и включает кнопку.

- `setTotal(total: number)` — Если сумма больше нуля — отображает её с форматированием (formatNumber) и надписью «синапсов»;
  Иначе — очищает текст суммы.

---
### [CardBasketView](src/components/view/basket/CardBasketView.ts)

представляет визуальное представление одной карточки товара в корзине, основанное на шаблоне AView. Он отображает индекс, название, цену и кнопку удаления для товара в корзине

***Поля***:
- `basketIndex: HTMLElement` — Элемент, в который выводится порядковый номер товара в корзине
- `cardTitle: HTMLElement` — Элемент, отображающий название товара.
- `cardPrice: HTMLElement` — Элемент, отображающий цену товара, отформатированную через formatNumber
- `cardButtonDelete: HTMLButtonElement` — Кнопка удаления карточки товара из корзины
-
***Конструктор***:

Принимает параметрами:
- `container: HTMLElement` — DOM-элемент, в который отрисовывается корзина;
- `actions: ICardBasketActions` — объект с обработчиком клика по кнопке удаления из корзины.
  Инициализирует DOM-элементы карточки:
  basketIndex — элемент с индексом товара
  cardTitle — название товара
  cardPrice — цена
  cardButtonDelete — кнопка удаления
  Назначает обработчик на кнопку удаления.

***Методы***:
- `doRender(data: Partial<ICardBasket>)` — Обновляет отображение карточки на основе переданных данных:
  index: отображается с помощью formatNumber
  price: также форматируется
  title: задаётся напрямую

---

## События
###  Каталог товаров

- `CHANGED = 'catalog:changed'` — Событие, которое вызывается при изменении каталога товаров.

### Модальные окна
// входящие события
- `PRODUCT_PREVIEW = 'modal:productPreview'` — Открывает модальное окно с карточкой товара (превью)
- `BASKET = 'modal:basket'` — Показывает корзину в модальном окне.
- `ORDER = 'modal:order'` — Показывает форму оформления заказа. 1 шаг.
- `CONTACTS = 'modal:contacts'` — Показывает форму с контактами пользователя. 2 шаг.
- `SUCCESS = 'modal:success'` — Показывает экран успешного завершения покупки.

// исходящие события
- `OPEN = 'modal:open'` — Модальное окно открылось.
- `CLOSE = 'modal:close'` — Модальное окно закрылось.
- `NONE = 'modal:none'` — Модальное окно полностью скрыто / неактивно (например, все виды контента закрыты).

### Корзина
// входящие события
- `ADD = 'basket:add'` — Добавить товар в корзину. Обычно сопровождается данными о товаре.
- `REMOVE = 'basket:remove'` — Удалить товар из корзины. Обычно передаётся идентификатор или индекс.
- `CLEAR = 'basket:clear'` — Полностью очистить корзину от всех товаров.

// исходящие события
- `CHANGED ='basket:changed'` — Корзина изменилась (добавление, удаление или очистка). Используется, чтобы обновить интерфейс или сохранить изменения.

### Оформление заказа
// входящие события
- `PAYMENT_SELECT = 'checkout:paymentSelect'` — Выбор способа оплаты (например, 'card' или 'cash').
- `ADDRESS_INPUT = 'checkout:addressInput'` — Ввод или изменение адреса доставки.

- `EMAIL_INPUT = 'checkout:emailInput'` — Ввод или изменение email.
- `PHONE_INPUT = 'checkout:phoneInput'` — Ввод или изменение номера телефона.

//исходящие события
- `ORDER_CHANGED = 'checkout:orderChanged'` — Изменение данных заказа на первом шаге: оплата, адрес, ошибки и валидность.
- `CONTACTS_CHANGED = 'checkout:contactsChanged'` — Изменение контактных данных на втором шаге: email, телефон, ошибки и валидность

### Успешный заказ

- `CHANGED = 'success:changed'` — Обновление данных успешного завершения заказа